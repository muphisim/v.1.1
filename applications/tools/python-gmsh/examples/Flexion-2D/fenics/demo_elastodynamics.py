...
# Define mesh
mesh = RectangleMesh(Point(0., 0.), Point(1., 0.1), 60, 10)
...
p = Expression(("0", "t <= tc ? p0*t/tc : 0"), t=0, tc=cutoff_Tc, p0=p0, degree=0)
...
p["Time"] = time
p["uy"] = u_tip
p.to_csv("dispyTip.csv",sep=",",index=False)


pp = pd.DataFrame()
pp["Time"] = time
pp["elastic"] = energies[:,0]
pp["kinetic"] = energies[:,1]
pp["damping"] = energies[:,2]
pp["total"] = energies[:,3]
pp.to_csv("energy.csv",sep=",",index=False)
