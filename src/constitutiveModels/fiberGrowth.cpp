...
//
//
/*!\file fiberGrowth.cpp
  \brief This file contains all functions related to a fiber growth constitutive model. There are two type of functions: the virtual ones inheritated from the general constitutive law (they MUST be implemented by the user), and the particular ones that the user wants to implement for his convenience.
*/
...
*/
classFiberGrowth::classFiberGrowth(int ndim, string name, string name_cons, double rho, double Young, double nu,
                                   double Gc) : constitutiveModels(ndim, name, name_cons, rho),
                _Young(Young),_nu(nu),_lambda((Young * nu) / ((1. + nu) * (1. - 2. * nu))),
                _Gc(Gc),_mu(0.5 * Young / (1. + nu)),_n0(_ndim)
 {
...
classFiberGrowth::~classFiberGrowth()
{
}

/*! \brief This function initialises this law from options
*/
void classFiberGrowth::initLawFromOptions()
{
...
*/
double classFiberGrowth::soundSpeed() const 
{
...
*/
void classFiberGrowth::initIntVars(vector<double> &intVars) {

    double theta0 = 1.0; // Initial value of the internal growth multiplier
    intVars.push_back(theta0);
}
...
*/
void classFiberGrowth::updateConstitutive(classGPs *GaussPoint, double dt, double timeRun, bool flagTanMod)
{
    ...
    vector<double> be(ndim *ndim,
    0);
    vector<double> kirch(ndim *ndim,
    0);
